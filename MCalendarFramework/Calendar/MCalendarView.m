...
- (UIButton *) preButton {
    if (!_preButton) {
        ...
        _preButton.userInteractionEnabled=NO;//交互关闭
        _preButton.alpha=0.4;//透明度
    }
    return _preButton;
}
...
- (UIView *) headerView {
    if (!_headerView) {
        ...
        for (int i = 0 ; i < 7; i++) {
            ...
            if ([label.text isEqualToString:@"周六"] || [label.text isEqualToString:@"周日"]) {
                 label.textColor = [UIColor colorWithHexString:@"#4180E9"];
            }else{
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSInteger weekday = [item.date weekday];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    int result = [NSDate compareOneDay:self.selectedDate withAnotherDay:selectedDate];
...
//    NSLog(@" == %@",[self.startDate preMonth]);
//
    
    NSDate *startDate = [self.startDate preMonth];
    NSDate *selectedDate = [NSDate date];
    int result = [NSDate compareOneDay:startDate withAnotherDay:selectedDate];
...
- (IBAction) nextMonthAction:(id)sender {
        ...
        NSDate *startDate = [self.startDate nextMonth];
        NSDate *selectedDate = [NSDate date];
        int result = [NSDate compareOneDay:startDate withAnotherDay:selectedDate];
        NSLog(@" result == %d",result);
        if (result == 1) {
            _preButton.userInteractionEnabled = YES;
            _preButton.alpha = 1;//透明度
        }
