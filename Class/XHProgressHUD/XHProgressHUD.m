...
//  JLRoutesMVVM_Demo
//
//  Created by power on 2019/9/20.
//  Copyright © 2019 yukechen. All rights reserved.
//
...
/// 提示框文字大小
static CGFloat kXWHUDDefaultFontSize = 14.0f;
...
+ (void)showHud:(NSString *)message customImgView:(UIView *)customImgView Mode:(MBProgressHUDMode)mode isShowMaskView:(BOOL)isShowMaskView afterDelay:(NSTimeInterval)delay onView:(UIView *)view completionBlock:(MBProgressHUDCompletionBlock)completionBlock{
    ...
    hud.label.font = [UIFont systemFontOfSize:kXWHUDDefaultFontSize];
    ...
    if (isShowMaskView) {
        ...
        hud.bezelView.style = MBProgressHUDBackgroundStyleSolidColor;
        hud.backgroundView.color = [[UIColor whiteColor]colorWithAlphaComponent:0.5];
//        hud.bezelView.backgroundColor = [UIColor redColor];
        hud.bezelView.color = [UIColor whiteColor];
//        hud.backgroundColor = [UIColor yellowColor];
        hud.customView.backgroundColor = [[UIColor whiteColor]colorWithAlphaComponent:0.5];
        
        hud.label.textColor = [UIColor colorWithHexString:@"#999999"];
    }
    // 隐藏的时候从父控件中移除
    hud.removeFromSuperViewOnHide = YES;
    // delay秒后隐藏
